...
"""
    ...
    def build_utility_from_discovery(self, tool_info):
        ...
        print("Building utility inspired by " + tool_info['name'] + "...")
        
        utility_name = "eliza_" + tool_info['name'].lower().replace('-', '_') + "_integration"
        
        # Generate simple utility code - SAFE VERSION
        utility_code = '# ' + tool_info['name'] + ' Inspired Utility\n'
        utility_code += '# Generated by Enhanced Eliza\n'
        utility_code += '# Inspired by: ' + tool_info['url'] + '\n'
        utility_code += '\n'
        utility_code += 'import json\n'
        utility_code += 'from datetime import datetime\n'
        utility_code += '\n'
        utility_code += 'class ElizaUtilityTool:\n'
        utility_code += '    def __init__(self):\n'
        utility_code += '        self.name = "' + tool_info['name'] + '_utility"\n'
        utility_code += '        self.created = datetime.now().isoformat()\n'
        utility_code += '        self.operations_log = []\n'
        utility_code += '\n'
        utility_code += '    def execute_operation(self, operation_type, data=None):\n'
        utility_code += '        operation = {\n'
        utility_code += '            "timestamp": datetime.now().isoformat(),\n'
        utility_code += '            "type": operation_type,\n'
        utility_code += '            "data": data,\n'
        utility_code += '            "status": "completed",\n'
        utility_code += '            "result": "Successfully executed " + str(operation_type)\n'
        utility_code += '        }\n'
        utility_code += '        self.operations_log.append(operation)\n'
        utility_code += '        return operation\n'
        utility_code += '\n'
        utility_code += '    def get_status(self):\n'
        utility_code += '        return {\n'
        utility_code += '            "utility_name": self.name,\n'
        utility_code += '            "operations_count": len(self.operations_log),\n'
        utility_code += '            "status": "active"\n'
        utility_code += '        }\n'
        utility_code += '\n'
        utility_code += 'if __name__ == "__main__":\n'
        utility_code += '    utility = ElizaUtilityTool()\n'
        utility_code += '    result = utility.execute_operation("initialization")\n'
        utility_code += '    print(json.dumps(result, indent=2))\n'
        
        # Create utility file
        success = self.commit_to_github(
            "utilities/" + utility_name + ".py",
            utility_code,
            "Built utility inspired by " + tool_info['name']
        )
        ...
        return utility_name    def __init__(self):
        self.name = "{tool_name}_utility"
        self.created = datetime.now().isoformat()
        self.operations_log = []
        
    def execute_operation(self, operation_type, data=None):
        # Execute a utility operation - FIXED
        operation = {{
